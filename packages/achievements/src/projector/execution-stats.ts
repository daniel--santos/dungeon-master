...
export interface ExecutionStatsState {
  readonly xp: number;
  ...
  readonly dockerRunsSucceeded: number;
...
export const EMPTY_EXECUTION_STATS: ExecutionStatsState = {
  xp: 0,
  level: 1,
  runsTotal: 0,
  runsSucceeded: 0,
 ...
 */
export type RunOutcome = "SUCCEEDED" | "FAILED" | "ABANDONED";
...
export function applyRunOutcome(
  ...
  if (input.outcome !== "SUCCEEDED") {
    return {
      ...
      runsFailed: input.outcome === "FAILED" ? state.runsFailed + 1 : state.runsFailed,
      harnessCounts,
    };
  }

  const primeiraSelada = input.executionMode === "DOCKER" && state.dockerRunsSucceeded === 0;

  const xp =
    state.xp +
    XP_PER_SUCCEEDED_RUN +
    (input.bug ? XP_BONUS_BUG_TASK : 0) +
  ...
  return {
    ...
    level: levelForXp(xp),
    runsTotal: state.runsTotal + 1,
    runsSucceeded: state.runsSucceeded + 1,
    bugTasksCompleted: input.bug ? state.bugTasksCompleted + 1 : state.bugTasksCompleted,
    dockerRunsSucceeded:
      input.executionMode === "DOCKER" ? state.dockerRunsSucceeded + 1 : state.dockerRunsSucceeded,
    harnessCounts,
  };
}

/** Soma os tokens de um evento `Usage`. Não mexe em XP: consumo não é feito. */
export function applyUsage(state: ExecutionStatsState, tokens: number): ExecutionStatsState {
